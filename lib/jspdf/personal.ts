...
export async function reportePersonal({
  ...
  doc.text("Reporte de personal", pageWidth / 2, 16, { align: "center" });
  doc.setFontSize(12);
  ...
  if (staff.length > 0) {
    const tableData = staff.map((personal, index) => [
      index + 1,
      ...
      new Intl.DateTimeFormat("es-ES", {
        ...
        .format(personal.staff?.contratation_date)
        .toString(),
      personal.role.role_name,
      personal.status === userStatusList.ACTIVO
        ? "activo"
        : personal.status === userStatusList.INACTIVO
          ? "inactivo"
          : personal.status === userStatusList.NUEVO
            ? "nuevo"
            : personal.status === userStatusList.BLOQUEADO
              ? "bloqueado"
              : "-",
      personal.created_at?.toLocaleDateString(),
    ]) as RowInput[];
    autoTable(doc, {
      head: [
        [
