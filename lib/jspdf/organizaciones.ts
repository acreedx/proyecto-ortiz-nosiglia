...
export async function reporteOrganizaciones({
  ...
  if (organizations.length > 0) {
    const tableData = organizations.map((organizacion, index) => [
      index + 1,
      organizacion.name,
      organizacion.address,
      organizacion.status === userStatusList.ACTIVO
        ? "activo"
        : organizacion.status === userStatusList.INACTIVO
          ? "inactivo"
          : "-",
      organizacion.created_at?.toLocaleDateString(),
    ]) as RowInput[];
    autoTable(doc, {
      head: [["#", "Nombre", "Dirección", "Estado", "Fecha de Registro"]],
      body: tableData,
      startY: 62,
      theme: "grid",
      headStyles: {
