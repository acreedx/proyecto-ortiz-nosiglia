...
import { appointmentReportData } from "../actions/operations";
import { reporteCitas } from "../../../../../lib/jspdf/citas";

export default function CreateAppointmentsReportForm() {
  const {
    register,
    handleSubmit,
  ...
  const onSubmit = async (data: TGenerateReportSchema) => {
    ...
    if (isConfirmed) {
      ...
      if (res.ok) {
        await reporteCitas({ data: data, appointments: res.citas });
        toaster.create({
